<?php 
    ...
    if(isset($EncodedData) && !empty($EncodedData))
    {
        ...
        $check = mysqli_fetch_array(mysqli_query($CN,$Sql_Query)) or die("database error:". mysqli_error($CN));
        
        if(isset($check))
        {
            $user_id = $check['id'];
            
            $qry="UPDATE `tbl_user` SET `deviceId`='$deviceId' WHERE `id`='{$user_id}' LIMIT 1  ";
